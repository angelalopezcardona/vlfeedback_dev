...
class LlamaForRewardModelWithGating(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.regression_layer = nn.Linear(
            config.hidden_size, config.num_quantiles * self.num_objectives, bias=False
        )
        self.post_init()
        # Not using torch.eye because it is not supported in BF16
        I = torch.zeros(self.num_objectives, self.num_objectives)
        I[range(self.num_objectives), range(self.num_objectives)] = 1.0
    ...
    def forward(
        ...
        # added from original model to output the attention
